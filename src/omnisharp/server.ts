...
export class OmniSharpServer {
    ...
    constructor(private vscode: vscode, networkSettingsProvider: NetworkSettingsProvider, private eventStream: EventStream, private packageJSON: any, private platformInfo: PlatformInformation, private optionStream: Observable<Options>) {
    ...
    public async restart(launchTarget: LaunchTarget = this._launchTarget): Promise<void> {
        if (launchTarget) {
            await this.stop();
            let options = await this.readOptions();
            await this._start(launchTarget, options);
        }
    }

    private readOptions(): Promise<Options>{
