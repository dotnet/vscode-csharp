...
import { getInstallableRuntimeDependencies } from './omnisharp/GetRuntimeDependenciesPackages';
...
async function ensureRuntimeDependencies(extension: vscode.Extension<CSharpExtensionExports>, eventStream: EventStream, platformInfo: PlatformInformation, networkSettingsProvider: NetworkSettingsProvider): Promise<boolean> {
    let installablePackages = getInstallableRuntimeDependencies(extension.packageJSON);
    return installCSharpExtDependencies(eventStream, platformInfo, networkSettingsProvider, installablePackages);
}
