...
export class RazorCSharpFeature {
    ...
    constructor(documentManager: RazorDocumentManager, eventEmitterFactory: IEventEmitterFactory, logger: RazorLogger) {
        this.projectionProvider = new CSharpProjectedDocumentContentProvider(
            documentManager,
            eventEmitterFactory,
            logger
        );
    ...
    public register() {
        const registrations = [
            vscode.workspace.registerTextDocumentContentProvider(
                CSharpProjectedDocumentContentProvider.scheme,
                this.projectionProvider
            ),
            vscode.commands.registerCommand('extension.showRazorCSharpWindow', async () =>
                this.csharpPreviewPanel.show()
            ),
        ];

        if (vscode.window.registerWebviewPanelSerializer) {
            registrations.push(
                vscode.window.registerWebviewPanelSerializer(CSharpPreviewPanel.viewType, {
