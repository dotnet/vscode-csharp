...
[true, false].forEach(useFramework => {
    suite(`OmnisharpDownloader (useFramework: ${useFramework})`, () => {
        ...
        setup(async () => {
            ...
            await server.start();
            server.addRequestHandler('GET', `/releases/${version}/omnisharp-win-x86${suffix}.zip`, 200, {
        ...
        test('Returns false if request is made for a version that does not exist on the server', async () => {
            expect(await downloader.DownloadAndInstallOmnisharp("1.00000001.0000", useFramework, server.baseUrl, installPath)).to.be.false;
        });
