...
describe(OmnisharpDownloader.name, () => {
    ...
    const suffix = getPackageSuffix(version);
    ...
    test('Returns false if request is made for a version that does not exist on the server', async () => {
        expect(await downloader.DownloadAndInstallOmnisharp('1.00000001.0000', server.baseUrl, installPath)).toBe(
            false
        );
    });
    ...
    test('Events are created', async () => {
        const expectedSequence = [
            ...
            new DownloadStart('OmniSharp for Windows (.NET / x64), Version = 1.2.3'),
            new DownloadSizeObtained(testZip.size),
            new DownloadProgress(100, 'OmniSharp for Windows (.NET / x64), Version = 1.2.3'),
            new DownloadSuccess(' Done!'),
            new InstallationStart('OmniSharp for Windows (.NET / x64), Version = 1.2.3'),
