...
sed -i "/await InstallZip(buffer, pkg.description, pkg.installPath, pkg.binaries, eventStream);/c                    if (pkg.url.includes('.tar.gz')) {                        await InstallTarGz(buffer, pkg.description, pkg.installPath, eventStream);                    } else {                        await InstallZip(buffer, pkg.description, pkg.installPath, pkg.binaries, eventStream);                    }" src/packageManager/downloadAndInstallPackages.ts
